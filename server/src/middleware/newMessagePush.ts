...
import { newMessagePushTitle, newMessagePushBody } from '../services/pushMessageFormat';
...
id: string | null;
  ...
  return messaging
    .send(message)
    .then((response: string) => console.log(`newMessage push notification sent to ${id} from ${otherId}: ${response}`))
    .catch((e: string) => console.log(`Error sending newMessage push notification to ${id} from ${otherId}: ${e}`));
};
