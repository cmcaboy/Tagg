...
exports.onLike = functions.firestore
  ...
  .onCreate(event => {
    ...
    db.collection(`users/${likedId}/likes`)
      .get()
      .then(likeList => {
...
const updateLastMessage = (matchId, lastMessage) => {
  return db
    ...
    .then(snapshot => {
      ...
      const lastMessageCall = db
        .collection(`users/${userA}/matches`)
        .doc(`${userB}`)
        .update({ lastMessage });
      const lastMessageCall2 = db
        .collection(`users/${userB}/matches`)
        .doc(`${userA}`)
        .update({ lastMessage });
...
const updateLastUser = (matchId, lastUser) => {
  return db
    ...
    .then(snapshot => {
      ...
      const lastUserCall = db
        .collection(`users/${userA}/matches`)
        .doc(`${userB}`)
        .update({ lastUser });
      const lastUserCall2 = db
        .collection(`users/${userB}/matches`)
        .doc(`${userA}`)
        .update({ lastUser });
...
exports.onMatchShowHide = functions.firestore
  ...
  .onUpdate(event => {
    ...
    if (newValue.active === 0 && oldValue.active === 1) {
      // Deactivate Matches
      db.collection(`users/${newValue.userA}/matches/`)
        .where("matchId", "==", matchId)
        .update({ active: 0 });
      db.collection(`users/${newValue.userB}/matches/`)
        .where("matchId", "==", matchId)
        .update({ active: 0 });
